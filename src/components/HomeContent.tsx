        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16 relative z-10">
          <div className="max-w-xl">
            ...
            <h1 className="text-4xl lg:text-5xl font-bold mb-4 leading-tight">
              <span className="text-gray-900">{t('site.name').slice(0, 2)}</span>
              <span className="text-gradient">{t('site.name').slice(2)}</span>
            </h1>
            <p className="text-xl text-gray-700 font-medium mb-2">{t('site.slogan')}</p>
            ...
            <div className="flex items-center flex-wrap gap-2 mb-8">
              <span className="text-sm text-gray-400">{t('hotSearch')}：</span>
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                { icon: '📚', value: stats.totalDocs.toLocaleString(), label: t('stats.docs') },
                { icon: '📁', value: stats.totalCategories.toLocaleString(), label: t('stats.categories') },
                { icon: '👥', value: stats.totalUsers.toLocaleString(), label: t('stats.users') },
                { icon: '⬇️', value: stats.totalDownloads.toLocaleString(), label: t('stats.downloads') },
              ].map((stat) => (
            ...
            <section className="mb-10">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1 ml-3">{t('categorySection.subtitle')}</p>
                </div>
                ...
                {categories.map((category) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-400 mt-1">{category.count}{t('sidebar.docs')}</p>
                    </Link>
                  );
                })}
              </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1 ml-3">{t('latestSection.subtitle')}</p>
                </div>
                <Link href="/category/行业标准" className="text-emerald-600 hover:text-emerald-700 transition-colors duration-200 text-sm font-medium">
                  {t('latestSection.viewAll')}
                </Link>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 truncate">{doc.description || t('site.description')}</p>
                      </div>
                      {/* 日期和按钮 */}
          ...
          <aside className="hidden xl:block w-72 flex-shrink-0">
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-100">
                <h3 className="font-bold text-gray-800 mb-4">{t('sidebar.advantages')}</h3>
                <div className="space-y-4">
                  {[
                    { icon: '🛡️', title: t('sidebar.reliable.title'), desc: t('sidebar.reliable.desc') },
                    { icon: '🔍', title: t('sidebar.smart.title'), desc: t('sidebar.smart.desc') },
                    { icon: '🔄', title: t('sidebar.realtime.title'), desc: t('sidebar.realtime.desc') },
                    { icon: '📄', title: t('sidebar.format.title'), desc: t('sidebar.format.desc') },
                  ].map((item) => (
              ...
              {/* 今日数据 */}
              <div className="bg-white rounded-2xl p-6 border border-gray-100">
                <h3 className="font-bold text-gray-800 mb-4">{t('sidebar.platformData')}</h3>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="text-xs text-gray-400 mb-1">{t('sidebar.totalDownloads')}</p>
                    <p className="text-xl font-bold text-gray-800">{stats.totalDownloads.toLocaleString()} <span className="text-xs font-normal text-gray-400">{t('sidebar.times')}</span></p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 mb-1">{t('sidebar.docsWithDownloads')}</p>
                    <p className="text-xl font-bold text-gray-800">{stats.todayUpdates} <span className="text-xs font-normal text-gray-400">{t('sidebar.docs')}</span></p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-gray-400 mb-1">{t('sidebar.totalDocs')}</p>
                    <p className="text-xl font-bold text-gray-800">{stats.totalDocs} <span className="text-xs font-normal text-gray-400">{t('sidebar.docs')}</span></p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 mb-1">{t('sidebar.registeredUsers')}</p>
                    <p className="text-xl font-bold text-gray-800">{stats.totalUsers} <span className="text-xs font-normal text-gray-400">{t('sidebar.people')}</span></p>
                  </div>
                </div>
              </div>
            </div>
