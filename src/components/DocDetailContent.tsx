...
export default function DocDetailContent({ document: doc, relatedDocs }: DocDetailContentProps) {
  ...
  // Format tag colors (gradient style)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="glass-strong rounded-3xl shadow-xl overflow-hidden animate-fade-in-up">
          <div className="p-8">
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8">
              ...
              <div className="flex flex-col items-center md:items-end space-y-4">
                <div className="text-center md:text-right">
                  {doc.price === 0 ? (
                    <span className="text-3xl font-bold text-emerald-500">{t('docDetail.free')}</span>
                  ) : (
                    <div>
                      <span className="text-sm text-gray-400">{t('docDetail.price')}</span>
                      <div className="flex items-center space-x-2">
                        <span className="text-yellow-400 text-2xl">⭐</span>
                        <span className="text-3xl font-bold text-primary-dark">{doc.price}</span>
                        <span className="text-gray-400">{t('docDetail.starCoin')}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="border-t border-white/30 pt-8">
              <h2 className="text-lg font-bold text-gray-700 mb-4">{t('docDetail.info')}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="glass rounded-2xl p-4">
                  <p className="text-sm text-gray-400 mb-1">{t('docDetail.pages')}</p>
                  <p className="text-lg font-medium text-gray-700">{doc.pages} {t('docDetail.pagesUnit')}</p>
                </div>
                <div className="glass rounded-2xl p-4">
                  <p className="text-sm text-gray-400 mb-1">{t('docDetail.fileSize')}</p>
                  <p className="text-lg font-medium text-gray-700">{doc.size}</p>
                </div>
                <div className="glass rounded-2xl p-4">
                  <p className="text-sm text-gray-400 mb-1">{t('docDetail.uploadTime')}</p>
                  <p className="text-lg font-medium text-gray-700">{doc.uploadDate}</p>
                </div>
                <div className="glass rounded-2xl p-4">
                  <p className="text-sm text-gray-400 mb-1">{t('docDetail.downloadCount')}</p>
                  <p className="text-lg font-medium text-gray-700">{doc.downloadCount} {t('docDetail.times')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {relatedDocs.length > 0 && (
          <section className="mt-12">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">{t('docDetail.related')}</h2>
